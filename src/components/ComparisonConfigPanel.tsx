...
interface ComparisonConfigPanelProps {
  selectedNodeId: string
  onClose: () => void
}

export default function ComparisonConfigPanel({ selectedNodeId, onClose }: ComparisonConfigPanelProps) {
  const afterNodes = useComparisonStore((s) => s.afterNodes)
  const updateAfterNode = useComparisonStore((s) => s.updateAfterNode)

  const selectedNode = afterNodes.find((n) => n.id === selectedNodeId) ?? null

  if (!selectedNode) return null

  const makePatch = <T extends NodeData>(patch: Partial<T>) => {
    updateAfterNode(selectedNodeId, patch as Partial<NodeData>)
  }

  return (
    <div className="absolute top-2 right-2 z-20 w-64 rounded-xl border border-zinc-800 bg-zinc-900 p-3 shadow-2xl">
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Configure (After)</p>
        ...
        <button
          ...
          aria-label="Close comparison config panel"
        >
      ...
      <div className="flex flex-col gap-3 max-h-96 overflow-y-auto">
        {/* Label field for non-device nodes */}
